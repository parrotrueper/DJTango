...
class milongaSource(QAbstractTableModel):
    ...
    def data(self, index, role=Qt.DisplayRole):
        if not index.isValid():
            return None
        if role == Qt.BackgroundRole:
            return self._background_role(index)
        if role == Qt.ForegroundRole:
            return self._foreground_role(index)
        if role != Qt.DisplayRole:
            return None
        return self._display_role(index)

    def _background_role(self, index):
        type_key = normalize_type_key(self.mylist[index.row()][5], self.TYPE)
        if type_key in self.TYPE:
            R = self.TYPE[type_key][2]
            G = self.TYPE[type_key][3]
    ...
    def _foreground_role(self, index):
        type_key = normalize_type_key(self.mylist[index.row()][5], self.TYPE)
        if type_key in self.TYPE:
            type_entry = self.TYPE[type_key]
            font_color = get_type_font_color(type_entry)
    ...
    def _display_role(self, index):
        ...
        if index.column() == 7:  # bpm column
            return f"{float(self.mylist[index.row()][index.column()]):.2f}"
        return self.mylist[index.row()][index.column()]
...
class milongaDest(QAbstractTableModel):
    ...
    def dropMimeData(self, data, action, row, column, parent):
        ...
        if action == Qt.MoveAction:
            ...
            rowItem = self._build_row_item(data_items)
            self._set_drop_data(beginRow, parent, data_items, rowItem)
        else:
            self._handle_drop_copy(beginRow, parent, data_items, lineNB)
            return True
    ...
    def _build_row_item(self, data_items):
        rowItem = {}
        for data in data_items:
            if data["row"] not in rowItem:
                rowItem[data["row"]] = data["row"]
        return rowItem

    def _set_drop_data(self, beginRow, parent, data_items, rowItem):
        curRow = 0
    ...
    def _handle_drop_copy(self, beginRow, parent, data_items, lineNB):
        beginRow += 1
        if beginRow <= self.rowCount(self):
            curRow = 0
        else:
            curRow = -1
        for _ in range(lineNB):
            self.insertRow(beginRow + _, parent)
