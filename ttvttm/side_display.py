...
from ttvttm.ui_utils import get_contrast_color, get_type_font_color
...
class SideDisplayMixin:
    def _handelDisplaySideScreen(self):
        ...
        geom, screen_count = self._get_side_screen_geometry()

        if self.sideWindow.isVisible():
            self._hide_side_window()
        else:
            self._show_side_window(geom, screen_count)

    def _get_side_screen_geometry(self):
        app = QGuiApplication.instance() or QGuiApplication([])
        screens = app.screens() if hasattr(app, "screens") else []
        main_screen = self._get_main_screen(app, screens)
        side_screen = self._get_side_screen(main_screen, screens)

        if side_screen is None:
            return QtCore.QRect(0, 0, 800, 600), len(screens)
        return side_screen.availableGeometry(), len(screens)

    def _get_main_screen(self, app, screens):
        main_screen = self.screen() if hasattr(self, "screen") else None
        if main_screen is None:
            main_screen = app.primaryScreen()
        if main_screen is None and screens:
            main_screen = screens[0]
        return main_screen

    def _get_side_screen(self, main_screen, screens):
        if len(screens) > 1 and main_screen is not None:
            return next((s for s in screens if s != main_screen), screens[0])
        return main_screen if main_screen is not None else (screens[0] if screens else None)

    def _show_side_window(self, geom, screen_count):
        if screen_count > 1:
