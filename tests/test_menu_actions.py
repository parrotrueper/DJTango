...
from djtango.qt_compat import QApplication, QColor, QDialog, QFont, QWidget, QtWidgets
from djtango.DJTango import AudioPlayerDialog
from djtango.gui_helpers import TrackAppearanceDialog
from djtango.menu_actions import LibraryContentsDialog
...
class FileMenuActionsTest(unittest.TestCase):
    ...
    def test_library_contents_action_opens_the_dialog(self):
        with tempfile.TemporaryDirectory() as tmp_home:
            ...
            with patch.dict(os.environ, env), patch('djtango.DJTango.QMediaPlayer', new=DummyPlayer), patch.object(AudioPlayerDialog, 'open_library_contents_dialog', new=fake_open_library_contents_dialog):
                dialog = AudioPlayerDialog()
    ...
    def test_exclude_library_path_removes_track_and_persists_exclusion(self):
        with tempfile.TemporaryDirectory() as tmp_home, tempfile.TemporaryDirectory() as library_dir:
            ...
            with patch.dict(os.environ, env), patch('djtango.DJTango.QMediaPlayer', new=DummyPlayer):
                dialog = AudioPlayerDialog()
    ...
    def test_track_appearance_menu_updates_type_and_calls_color_font_handlers(self):
        with tempfile.TemporaryDirectory() as tmp_home:
            ...
            with patch.dict(os.environ, env), patch('djtango.DJTango.QMediaPlayer', new=DummyPlayer), patch.object(AudioPlayerDialog, '_showInfo', new=fake_show_info), patch.object(AudioPlayerDialog, '_updateSideScreen', new=fake_update_side_screen), patch.object(TrackAppearanceDialog, 'exec', new=fake_exec):
                dialog = AudioPlayerDialog()
    ...
    def test_track_appearance_dialog_choose_font_button_updates_preview_font(self):
        ...
        with patch('djtango.gui_helpers.QtWidgets.QFontDialog', new=DummyFontDialog):
    ...
    def test_track_appearance_apply_updates_library_table_font(self):
        ...
        parent._dialog.milongaSource = QtWidgets.QTableView(parent)
        parent._dialog.milongaDest = QtWidgets.QTableView(parent)
